...
- F(c)：新浪 hf_NQ 行情行的昨结算，作结算锚点 S(c) 的代理；F(t)：同一行情行的买卖价中点，按成员报价时刻 as-of 取。
  S(c) 不是"与 16:00 ET 指数收盘同刻的成交价"：CME 对主力合约按 14:59:30—15:00 CT 成交 VWAP 结算并有无成交回退规则，
  其他月份另有规则；供应商字段的日期与数值未经官方确认。因此 F(t)/S(c) 相对指数涨跌的误差含基差变化与锚点差异
  F(c)/S(c) 两项，亚洲时点均未识别（reports/phase0/futures/findings.md 第 9 节）。
