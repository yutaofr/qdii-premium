...
### 18.4 五审修正（见 reports/mvp/review-2026-09-16-round5-response.md）

| ADR | 决策 | 理由 |
|---|---|---|
| 039 | 辅助任务（状态页、主机探测、锚点捕获）由监督协程独立重启：退避 5→15→30→60 秒，稳定运行 120 秒后重置退避；失败写入 `health.degraded` 并在状态页告警，恢复后清除并累计重启次数。采集任务仍然整体退出交给 launchd | 五审 E1：只隔离不重启时，状态页可能整个窗口不可用，用户连降级告警都看不到 |

| ADR | 决策 | 理由 |
|---|---|---|
| 040 | 换月窗口（季月到期前 11 天至到期日）内始终披露 ROLL_WINDOW；输入包记录 `roll_window` | 实测新浪连续代码在季月到期前 1—3 天切换，基差跳升 +51—165bp。换月当天盘中报价已是新合约而昨结算仍是旧合约，`F(t)/F(c)` 会凭空多出约 1% |
| 041 | 估算固定使用**身份已知**的季月合约 `hf_NQYYMM`（距到期至少 3 天的最近一个），`F(t)` 与 `F(c)` 取自该合约同一行情行；连续代码 `hf_NQ` 只作对照与兜底。退回连续代码且处于换月窗口时一律不出估算（ROLL_ANCHOR_MISSING）。另设 3% 异常熔断（SOURCE_ANCHOR_MISMATCH），只作异常检测，不作身份凭据。输入包记录 `futures_contract`（schema 6） | D4 专项 F1：净变动无法拆分合约价差与行情变动（例：价差 +1% 与行情 −0.6% 相抵后净变动仅 +0.4%），阈值不能证明同合约。2026-09-16 实测新浪提供带月份的单一合约，`hf_NQ2609` 与连续代码逐字段相同、`hf_NQ2612` 高约 1.0%，因此可以直接固定合约（DS-09"优先固定一个已验证月份合约"） |
