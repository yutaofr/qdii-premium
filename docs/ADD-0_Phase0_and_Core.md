...
| ADR | 决策 | 理由 |
|---|---|---|
| 040 | 换月窗口（季月到期前 11 天至到期日）内：估算始终披露 ROLL_WINDOW；`\|F(t)/F(c) − 1\|` 超过 0.6% 时判 ROLL_ANCHOR_MISSING，不出估算。输入包记录 `roll_window`（schema 5） | 实测新浪连续合约在季月到期前 1—3 天切换，基差跳升 +51—165bp（6/6 季度一致，见 reports/phase0/futures/findings.md）。换月当天盘中报价已是新合约而昨结算仍是旧合约，`F(t)/F(c)` 会凭空多出约 1% |
